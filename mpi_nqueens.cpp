...
void worker_main() {
    ...
    while (true) {
        ...
        // If we get the termination signal, exit the loop - see http://stackoverflow.com/questions/10490983/mpi-slave-processes-hang-when-there-is-no-more-work
        if (stat.MPI_TAG == TERMINATION_MSG) {
            // printf("Process %d got the termination signal; exiting work loop.\n", rank);
            break;
        }

        // Clear the worker cache first, or repeat answers will be sent!!!
        WorkerSolutionStore::clear_solutions();
